...
#'                sampler = torch.utils.data.distributed.DistributedSampler(self.train)
                return DataLoader(self.train, batch_size=self.bs, num_workers = 4, pin_memory=True)#, sampler=sampler)

        def val_dataloader(self):
#                sampler = torch.utils.data.distributed.DistributedSampler(self.val)
                return DataLoader(self.val, batch_size=self.bs, num_workers = 4, pin_memory=True)#, sampler=sampler)

        def test_dataloader(self):
#                sampler = torch.utils.data.distributed.DistributedSampler(self.test)
                return DataLoader(self.test, batch_size=self.bs, num_workers = 4, pin_memory=True)#, sampler=sampler)

        def predict_dataloader(self):
                return DataLoader(self.test, batch_size=128, num_workers=4)